...
class FtCtrl extends DexpaprikaControl
{
...
class FtClient
{
    ...
    private DexpaprikaUtility $utility;

    public function __construct(DexpaprikaUtility $utility, array $options)
    {
    ...
    public function get_utility(): DexpaprikaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DexpaprikaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DexpaprikaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DexpaprikaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DexpaprikaConfig::make_feature($name);
    ...
    private static function buildUrl(DexpaprikaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DexpaprikaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DexpaprikaResponse($fetched);
        }
    ...
    private function fail(DexpaprikaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DexpaprikaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
