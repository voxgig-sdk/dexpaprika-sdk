...
func TestTokenEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Token(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Token(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DEXPAPRIKA_TEST_TOKEN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tokenRef01Match := map[string]any{}

		tokenRef01ListResult, err := tokenRef01Ent.List(tokenRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, tokenRef01ListOk := tokenRef01ListResult.([]any)
		if !tokenRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", tokenRef01ListResult)
		}

		// LOAD
		tokenRef01MatchDt0 := map[string]any{
			"id": tokenRef01Data["id"],
		}
		tokenRef01DataDt0Loaded, err := tokenRef01Ent.Load(tokenRef01MatchDt0, nil)
		if err != nil {
		...
		if tokenRef01DataDt0LoadResult["id"] != tokenRef01Data["id"] {
...
func tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DEXPAPRIKA_TEST_TOKEN_ENTID")
