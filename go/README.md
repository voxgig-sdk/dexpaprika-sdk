# Dexpaprika Golang SDK



The Golang SDK for the Dexpaprika API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/dexpaprika-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/dexpaprika-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/dexpaprika-sdk/go=../dexpaprika-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *DexpaprikaSDK
```
...
| `Token` | `(data map[string]any) DexpaprikaEntity` | Create a Token entity instance. |

### Entity interface (DexpaprikaEntity)

All entities implement the `DexpaprikaEntity` interface.
...
slice):
    ...
    exchange, err := client.Exchange(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // exchange is the returned record
...
Create an instance: `exchange := client.Exchange(nil)`
...
Create an instance: `pool := client.Pool(nil)`
...
```
github.com/voxgig-sdk/dexpaprika-sdk/go/
├── dexpaprika.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/dexpaprika-sdk/go`) re-exports everything needed
