...
//     (\`new DexpaprikaSDK(...)\` / \`DexpaprikaSDK.test(...)\` become
//     \`DexpaprikaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"exchange":{"test01":{"id":"test01"}},"historical":{"test01":{"id":"test01"}},"pool":{"test01":{"id":"test01"}},"ticker":{"test01":{"id":"test01"}},"token":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DexpaprikaSDK(...)\` and any
// \`DexpaprikaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DexpaprikaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DexpaprikaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
