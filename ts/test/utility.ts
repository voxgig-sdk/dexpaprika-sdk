...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DEXPAPRIKA_TEST_LIVE ||
    'TRUE' === process.env.DEXPAPRIKA_TEST_OVERRIDE
  ) {
  ...
  m.DEXPAPRIKA_TEST_EXPLAIN = process.env.DEXPAPRIKA_TEST_EXPLAIN || m.DEXPAPRIKA_TEST_EXPLAIN
