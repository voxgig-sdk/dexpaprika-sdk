...
local ExchangeEntity = {}
ExchangeEntity.__index = ExchangeEntity


function ExchangeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExchangeEntity)
  self._name = "exchange"
  self._client = client
...
function ExchangeEntity:get_name()
  return self._name
end


function ExchangeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExchangeEntity.new(self._client, opts)
end
...
function ExchangeEntity:mark_deleted()
  self._deleted = true
end


function ExchangeEntity:deleted()
  return true == self._deleted
end


function ExchangeEntity:data_set(args)
  if args ~= nil then
...
function ExchangeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExchangeEntity:match_set(args)
  if args ~= nil then
...
function ExchangeEntity:match_get()
...
function ExchangeEntity:stream(action, args, callopts)
...
function ExchangeEntity:_run_op(ctx, post_done)
  local utility = self._utility
