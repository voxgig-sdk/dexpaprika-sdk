-- Dexpaprika SDK Pool entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PoolEntity = {}
PoolEntity.__index = PoolEntity


function PoolEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PoolEntity)
  self._name = "pool"
  self._client = client
...
function PoolEntity:get_name()
  return self._name
end


function PoolEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PoolEntity.new(self._client, opts)
end
...
function PoolEntity:mark_deleted()
  self._deleted = true
end


function PoolEntity:deleted()
  return true == self._deleted
end


function PoolEntity:data_set(args)
  if args ~= nil then
...
function PoolEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PoolEntity:match_set(args)
  if args ~= nil then
...
function PoolEntity:stream(action, args, callopts)
...
function PoolEntity:_run_op(ctx, post_done)
  local utility = self._utility
