...
describe("TokenDirect", function()
  it("should direct-list-token", function()
    local setup = token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-token", setup.live and "live" or "unit")
  ...
  it("should direct-load-token", function()
    local setup = token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function token_direct_setup(mockres)
