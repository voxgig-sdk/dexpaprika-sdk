...
The Lua SDK for the Dexpaprika API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Exchange(nil):list(nil, nil)
...
local result, err = client:Dexpaprika(nil):load(
  { id = "test01" }, nil
)
