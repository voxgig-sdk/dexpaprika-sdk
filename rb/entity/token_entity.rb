...
class TokenEntity
  ...
  def data_set(args)
    if args
      @_data = DexpaprikaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DexpaprikaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Token.
  #
  # @param reqmatch [TokenLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Token.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Token, Hash] the loaded Token; raises DexpaprikaError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DexpaprikaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TokenListMatch, Hash, nil] match filter (any subset of
  #   Token fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Token>, Array] the matching Token items; raises DexpaprikaError on failure
