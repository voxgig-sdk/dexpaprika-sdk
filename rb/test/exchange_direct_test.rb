...
class ExchangeDirectTest < Minitest::Test
  def test_direct_list_exchange
    setup = exchange_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-exchange", setup[:live] ? "live" : "unit")
    if _should_skip
...
def exchange_direct_setup(mockres)
