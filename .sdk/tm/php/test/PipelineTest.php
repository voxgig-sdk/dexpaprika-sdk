...
{
    private static function utility(): DexpaprikaUtility
    {
        return new DexpaprikaUtility();
    }
    ...
    private static function ctx(array $over = []): DexpaprikaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DexpaprikaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DexpaprikaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DexpaprikaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DexpaprikaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DexpaprikaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DexpaprikaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DexpaprikaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DexpaprikaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DexpaprikaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DexpaprikaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DexpaprikaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DexpaprikaResponse([]),
            'result' => new DexpaprikaResult([]),
        ]);
        [, $err] = DexpaprikaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec([]),
            'response' => null,
            'result' => new DexpaprikaResult([]),
        ]);
        [, $err] = DexpaprikaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec(['step' => 's']),
            'response' => new DexpaprikaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DexpaprikaResult([]),
        ]);
        [, $err] = DexpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec(['step' => 's']),
            'response' => new DexpaprikaResponse(self::resp(200, ['v' => 1])),
            'result' => new DexpaprikaResult([]),
        ]);
        [, $err] = DexpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec(['step' => 's']),
            'response' => new DexpaprikaResponse(self::resp(200, ['v' => 2])),
            'result' => new DexpaprikaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DexpaprikaMakeResponse::call($ctx);
    ...
    {
        $preset = new DexpaprikaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec([]),
            'response' => new DexpaprikaResponse([]),
            'result' => new DexpaprikaResult([]),
        ...
        [$got, $err] = DexpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DexpaprikaResult([])]);
        [, $err] = DexpaprikaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DexpaprikaSpec([]), 'result' => null]);
        [, $err] = DexpaprikaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DexpaprikaSpec(['step' => 's']),
            'result' => new DexpaprikaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DexpaprikaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DexpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DexpaprikaSpec(['step' => 's']),
            'result' => new DexpaprikaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DexpaprikaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DexpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DexpaprikaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DexpaprikaSpec([]),
            'result' => new DexpaprikaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DexpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DexpaprikaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DexpaprikaError('boom', 'boom');
        $utility->fetcher = function (DexpaprikaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DexpaprikaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DexpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DexpaprikaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DexpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DexpaprikaResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DexpaprikaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DexpaprikaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DexpaprikaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DexpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DexpaprikaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DexpaprikaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DexpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DexpaprikaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DexpaprikaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DexpaprikaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DexpaprikaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DexpaprikaResult(['ok' => false])]);
        $this->expectException(DexpaprikaError::class);
        DexpaprikaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DexpaprikaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DexpaprikaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DexpaprikaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DexpaprikaBaseFeature();
        $b = new DexpaprikaBaseFeature();
        DexpaprikaFeatureAdd::call($ctx, $a);
        DexpaprikaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DexpaprikaBaseFeature
    {
        $f = new DexpaprikaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DexpaprikaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DexpaprikaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DexpaprikaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DexpaprikaPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DexpaprikaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DexpaprikaResponse(['headers' => null]),
            'result' => new DexpaprikaResult([]),
        ]);
        DexpaprikaResultHeaders::call($ctx);
