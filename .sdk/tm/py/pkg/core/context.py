...
from projectname_sdk.core.control import DexpaprikaControl
...
class DexpaprikaContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, DexpaprikaSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DexpaprikaResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DexpaprikaResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return DexpaprikaError(code, msg, self)
