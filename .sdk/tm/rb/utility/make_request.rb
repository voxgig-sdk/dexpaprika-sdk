...
module DexpaprikaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DexpaprikaResponse.new({})
    ...
    if fetch_err
      ...
      response = DexpaprikaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DexpaprikaResponse.new(fetched)
    else
