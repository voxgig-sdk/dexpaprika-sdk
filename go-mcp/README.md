...
[MCP](https://modelcontextprotocol.io) server exposing the Dexpaprika SDK as
two agent tools — `dexpaprika_list` and `dexpaprika_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dexpaprika-mcp)
make build

# 2. Provide credentials via the environment
export DEXPAPRIKA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dexpaprika \
  -- /absolute/path/to/dexpaprika-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dexpaprika-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dexpaprika_list: first page of records
{ "entity": "exchange" }
{ "entity": "exchange", "query": { } }

// dexpaprika_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dexpaprika-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dexpaprika \
     -- "$PWD"/dist/*/dexpaprika-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dexpaprika_list` and `dexpaprika_load` tools now appear
   in new sessions. Ask the agent to *"list exchange using dexpaprika"*
   and it calls `dexpaprika_list` with `{"entity":"exchange"}`.
...
```sh
export DEXPAPRIKA_APIKEY=sk_live_xxx            # API key
export DEXPAPRIKA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dexpaprika_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dexpaprika_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DEXPAPRIKA_APIKEY` | API key sent with every request. |
| `DEXPAPRIKA_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

exchange | historical | pool | ticker | token

### Smoke test via HTTP (raw JSON-RPC)

```sh
./dexpaprika-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dexpaprika_load","arguments":{"entity":"historical","query":{"id":1}}}}'
```
