...
type Args struct {
	Entity string         `json:"entity" jsonschema:"exchange | historical | pool | ticker | token"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DexpaprikaSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "dexpaprika_list",
		Description: "List records from Dexpaprika. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "dexpaprika_load",
		Description: "Load a single record from Dexpaprika. " +
...
func runOp(client *sdk.DexpaprikaSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DexpaprikaSDK, name string) (sdk.DexpaprikaEntity, error) {
	switch strings.ToLower(name) {
	case "exchange":
		return client.Exchange(nil), nil
	case "historical":
