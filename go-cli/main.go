// Command Dexpaprika-cli is a CLI + REPL for the Dexpaprika SDK,
...
const prompt = "dexpaprika"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "exchange historical pool ticker token"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DEXPAPRIKA_APIKEY (api key), DEXPAPRIKA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DEXPAPRIKA_APIKEY carries the API key and
	// DEXPAPRIKA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DEXPAPRIKA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DEXPAPRIKA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDexpaprikaSDK(opts)
