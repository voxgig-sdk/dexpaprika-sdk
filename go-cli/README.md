...
boru-driven command-line client **and** interactive REPL for the Dexpaprika
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dexpaprika-cli)
make build
...
./dexpaprika-cli list exchange
./dexpaprika-cli list historical

# 5. Override the API base URL for a single call
DEXPAPRIKA_BASE=https://api.example.com ./dexpaprika-cli list exchange

# 6. No arguments -> interactive REPL
./dexpaprika-cli
dexpaprika> list exchange
dexpaprika> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dexpaprika-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dexpaprika-cli list exchange
   ```
...
Run with no arguments to open a REPL (prompt `dexpaprika>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `exchange`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dexpaprika-cli`. |
...
(`./dexpaprika-cli <expr>`) and interactively (the REPL), and expressions compose the
