...
class TestExchangeDirect:

    def test_should_direct_list_exchange(self):
        setup = _exchange_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-exchange", "live" if setup["live"] else "unit")
        if _skip:
...
def _exchange_direct_setup(mockres):
