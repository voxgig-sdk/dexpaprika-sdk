...
class TestPoolDirect:
    ...
    def test_should_direct_list_pool(self):
        setup = _pool_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-pool", "live" if setup["live"] else "unit")
        if _skip:
...
def _pool_direct_setup(mockres):
