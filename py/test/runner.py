...
class DexpaprikaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DexpaprikaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DexpaprikaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DexpaprikaTestRunner.getenv("DEXPAPRIKA_TEST_LIVE")
        override = DexpaprikaTestRunner.getenv("DEXPAPRIKA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DexpaprikaTestRunner.getenv(key)
        ...
        explain = DexpaprikaTestRunner.getenv("DEXPAPRIKA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DEXPAPRIKA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DexpaprikaTestRunner._test_control is not None:
            return DexpaprikaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DexpaprikaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DexpaprikaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DexpaprikaTestRunner.load_test_control()
...
def load_env_local():
    DexpaprikaTestRunner.load_env_local()


def env_override(m):
    return DexpaprikaTestRunner.env_override(m)


def entity_data(v):
    return DexpaprikaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DexpaprikaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DexpaprikaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DexpaprikaTestRunner.load_test_control()


def live_delay_ms():
    return DexpaprikaTestRunner.live_delay_ms()
