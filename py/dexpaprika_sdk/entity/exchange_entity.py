...
from dexpaprika_sdk.dexpaprika_types import (
    Exchange,
    ExchangeListMatch,
)
...
class ExchangeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ExchangeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Exchange]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Exchange().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
