...
The Python SDK for the Dexpaprika API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Exchange(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Dexpaprika(None).load(
    {"id": "test01"}, None
)
